...
def _make_parser():
    ...
    def validate_file_input(f) -> pathlib.Path:
        ...
        if not path.exists():
            raise argparse.ArgumentTypeError(f"Directory or file does not exist: {f}")
    ...
    def validate_output_dir(f) -> pathlib.Path:
        ...
        if not path.exists():
            raise argparse.ArgumentTypeError(f"Output directory does not exist: {f}")
...
def create_er_list(tree: etree.ElementTree) -> list[list[str, str]]:
    """
    ...
    for child in tree:
        ...
        if level >= 0:
            ...
            if level <= len(hierarchy) - 1:
                ...
                raise ValueError(f"Unexpected jump in hierarchy at {child.text}")
            ...
            if possible_ref and hierarchy[-1].startswith("ER"):
                ...
                ers.append(["/".join(hierarchy.copy()), refid, hierarchy[-1]])
...
def audit_ers(ers: list[list[str, str, str]]) -> None:
    ...
    for i in range(er_min, er_max):
        if i not in er_numbers_used.keys():
            LOGGER.warning(
                (
                    f"Collection uses ER {er_min} to ER {er_max}. ER {i} is skipped. "
    ...
    for er_number, er_names in er_numbers_used.items():
        if len(er_names) > 1:
            LOGGER.warning(
                (
                    f'ER {er_number} is used multiple times: {", ".join(er_names)}. '
                    "Review the ERs with the processing archivist"
                )
            )
...
def transform_bookmark_tables(tree: etree.ElementTree) -> list[dict]:
    """
...
def add_extents_to_ers(
    er_list: list[list[str, str]], bookmark_tables: list[dict]
    ...
    for er in er_list:
        ...
        if count == 0:
            LOGGER.warning(
                (
                    f"{er_name} does not contain any files. "
        ...
        if size == 0:
            LOGGER.warning(
                (
                    f"{er_name} contains no files with bytes. "
                    "This ER is omitted from report. "
                    "Review this ER with the processing archivist."
                )
            )
            continue
...
def get_er_report(
    er_files: list[dict], bookmark_id: str, er_name: str
) -> tuple([int, int]):
    """
        ...
        if entry["bookmark_id"] == prefix:
            ...
            if bytes:
                ...
                if file_size == 0:
                    ...
                    LOGGER.warning(
                        (f"{er_name} contains the following 0-byte file: {file_name}. "
...
def create_report(input: list[str, int, int], report: dict) -> dict:
    """
...
def extract_collection_title(tree: etree.ElementTree) -> str:
...
def make_json(destination: pathlib.Path, report: dict, collname) -> None:
    """
