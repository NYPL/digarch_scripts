...
def parse_args():
    ...
    def validate_dir(d: str) -> pathlib.Path:
        path = pathlib.Path(d)
        if not path.exists():
...
def get_ers(facomponent_dir: pathlib.Path) -> list[str, int, int, str]:
    ers = []
    for possible_er in facomponent_dir.glob("**/ER *"):
        ...
        if possible_er.is_dir():
            if objects_dir.is_dir():
                    ...
                    for f in files:
                        ...
                        if os.path.getsize(fp) == 0:
                            LOGGER.warning(
                                (
                                    f"{possible_er.name} "
                                    "contains the following 0-byte file: {f}. "
                ...
                LOGGER.warning(
                    (
                        f"{possible_er.name} does not contain an object folder. "
        ...
        if count == 0:
            LOGGER.warning(
                (
                    f"{possible_er.name} does not contain any files. "
                    "It will be omitted from the report."
                )
            )
            continue
        if size == 0:
            LOGGER.warning(
                (
                    f"{possible_er.name} contains no files with bytes. "
...
def extract_collection_title(hdd_dir: pathlib.Path) -> str:
    for item in hdd_dir.iterdir():
        if re.match(r"M\d+\_FAcomponents", item.name):
            ...
            LOGGER.warning(
                (
                    "Cannot find CollectionID_FAcomponents directory. "
                    "Please use CollectionID_FAcomponents naming convention "
                    "for the directory containing all ERs."
                )
            )
...
def create_report(input: list[list[str, int, int]], report: dict) -> dict:
    for er in input:
...
def process_item(input: list[str, int, int], report: dict) -> dict:
    if "/" not in input[0]:
...
def write_report(report: dict, dest: pathlib.Path) -> None:
