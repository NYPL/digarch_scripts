...
def test_hierarchy_nests_down_correctly(parsed_report):
    ...
    assert (
        "Extents Test papers/Series 1/Subseries(1)/Subsubseries(2)/ER 2: File 15, 2023"
        in just_titles
    )
...
def test_hierarchy_nests_empty_subseries(parsed_report):
    ...
    assert (
        "Extents Test papers/Series 1/Subseries(1)/Subsubseries(2)/Subsubsubseries(3)"
...
def test_hierarchy_nests_up_correctly(parsed_report):
    ...
    assert (
        "Extents Test papers/Series 1/Subseries(1)/Subsubseries(2) the second/ER 23: "
        "File 17, 2023"
        in just_titles
    )
    assert (
        "Extents Test papers/Series 1/Subseries(1) the second/ER 4: File 18, 2023"
        in just_titles
    )
...
def test_hierarchy_nests_reverse_order_bookmarks(parsed_report):
    ...
    assert (
        "Extents Test papers/Series 2/Subseries(1) of Series 2/ER 8: File 2, 2023"
        in just_titles
    )
    assert (
        "Extents Test papers/Series 2/Subseries(1) of Series 2/Subsubseries(2) of "
        "Series 2/ER 7: File 19, 2023"
        in just_titles
    )
...
def test_warn_on_no_files_in_er(parsed_report, caplog):
    ...
    log_msg = f"{er_with_no_files[0][0]} does not contain any files. It will be "
...
def test_warn_on_a_no_byte_file_in_er(parsed_report, caplog):
    ...
    log_msg = f"{er_with_no_bytes[0][0]} contains the following 0-byte file: "
    "file00.txt. Review this file with the processing archivist."
    assert log_msg in caplog.text
...
def test_warn_on_no_bytes_in_er(parsed_report, caplog):
    ...
    log_msg = f"{er_with_no_bytes[0][0]} contains no files with bytes. This ER is "
    "omitted from report. Review this ER with the processing archivist."
    assert log_msg in caplog.text
...
def test_skipped_ER_number_behavior(parsed_report, caplog):
    ...
    for i in range(13, 23):
        assert (
            f"Collection uses ER 1 to ER 23. ER {i} is skipped. Review the ERs with "
...
def test_repeated_ER_number_behavior(parsed_report, caplog):
    ...
    log_msg = ("ER 10 is used multiple times: ER 10: File 21,2023, "
               "ER 10: Folder 2, 2023. Review the ERs with the processing archivist"
               )
    assert log_msg in caplog.text
