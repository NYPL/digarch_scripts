...
def test_hierarchy_nests_up_correctly(parsed_report):
    ...
    assert [
        ...
        "Series 1",
        "Subseries(1)",
        "Subsubseries(2) the second",
        "ER 23: File 17, 2023",
    ] in just_titles
    assert [
        "Extents Test papers",
        "Series 1",
        "Subseries(1) the second",
        "ER 4: File 18, 2023",
    ] in just_titles
...
def test_warn_on_a_no_byte_file_in_er(parsed_report, caplog):
    ...
    er_with_no_bytes = [
        [["hier", "archy", "list"], "bk28001", "ER 6: Zero Length, 2023"]
    ]
...
def test_skipped_ER_number_behavior(parsed_report, caplog):
    ...
    for i in range(13, 23):
        assert (
            f"Collection uses ER 1 to ER 23. ER {i} is skipped. Review the ERs with the processing archivist"
            in caplog.text
        )
