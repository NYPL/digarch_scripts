...
./droid-binary-6.2.1-bin/droid.sh -p "/Volumes/Staging/MISC/$MediaID.droid" -n "File count and sizes" -t "DROID Report XML" -r "/Volumes/Staging/ingest/fileTransfers/$Collection/$MediaID/metadata/$MediaID.xml"
./droid-binary-6.2.1-bin/droid.sh -p "/Volumes/Staging/MISC/$MediaID.droid" -e "/Volumes/Staging/ingest/fileTransfers/$Collection/$MediaID/metadata/$MediaID.csv"
#end hide
exec 2>&3
if [ -n "$(find /Volumes/Staging/ingest/fileTransfers/$Collection/$MediaID/metadata/ -maxdepth 1 -name "*.xml")" ]
then
	echo -e "${BLUE}DROID XML has been created in metadata directory.${NC}"
else
	echo -e "${BLUE}DROID XML ERROR.${NC}"
fi
if [ -n "$(find /Volumes/Staging/ingest/fileTransfers/$Collection/$MediaID/metadata/ -maxdepth 1 -name "*.csv")" ]
then
